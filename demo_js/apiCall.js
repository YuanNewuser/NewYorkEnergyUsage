function apiCall(balances) {
    ...
    obj = {
        title:
            languageNameSpace.labels["tableYear"] + ": " + REF.year + "<br>" + languageNameSpace.labels["tableUnit"] + ": " + REF.unit,
    };
    column.push(obj);
...
function newRowApiCall(d) {
    ...
    res.forEach(function (value, i) {
        url[i] =
            "https://ec.europa.eu/eurostat/wdds/rest/data/v2.1/json/en/nrg_bal_c?";
        url[i] += "&geo=" + REF.geo;
        for (var bIdx = 0; bIdx < value.length; bIdx++) {
            url[i] += "&nrg_bal=" + value[bIdx];
        }
